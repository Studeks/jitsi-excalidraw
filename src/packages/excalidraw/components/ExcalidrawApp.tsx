...
const ExcalidrawWrapper = (props: ExcalidrawAppProps) => {
  ...
  const onChange = (
    ...
    // NUCLEAR OPTION: Check for problematic images in collaboration mode
    if (collabAPI?.isCollaborating() && props.onFileFetch) {
      const imageElements = elements.filter(
        (el) => isInitializedImageElement(el) && !el.isDeleted,
      );
      const problematicImages = imageElements.filter((el) => {
        const fileId = (el as any).fileId;
        const hasFile = !!files[fileId];
      ...
      if (problematicImages.length > 0) {
        console.log(
          "🚨 NUCLEAR onChange: Found problematic images:",
          problematicImages.map((el) => ({
            id: el.id,
            fileId: (el as any).fileId,
            status: (el as any).status,
            hasFile: !!files[(el as any).fileId],
          })),
        );

        const fileIdsToFetch = problematicImages.map(
          (el) => (el as any).fileId,
        );

        // Debounced fetch to avoid spamming
        if (!(onChange as any)._fetchTimeout) {
          (onChange as any)._fetchTimeout = setTimeout(() => {
            console.log(
              "🚀 NUCLEAR onChange: Fetching images:",
              fileIdsToFetch,
            );
            props.onFileFetch!(fileIdsToFetch)
              .then((response) => {
                console.log("✅ NUCLEAR onChange: Fetch response:", response);
                if (excalidrawAPI) {
                  excalidrawAPI.addFiles(response.loadedFiles);

                  // Force update element statuses
                  const updatedElements = excalidrawAPI
                    ...
                    .map((element) => {
                      if (
                        ...
                        if (hasLoadedFile) {
                          console.log(
                            "🔄 NUCLEAR onChange: Updating element status to saved:",
                            element.id,
                            fileId,
                          );
              ...
              .catch((error) => {
                console.error("❌ NUCLEAR onChange: Fetch failed:", error);
              })
              .finally(() => {
                (onChange as any)._fetchTimeout = null;
              });
          }, 100);
        }
      }
